...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $datos = [
        ...
        'dpt_desempeñar' => strtoupper(sanitizeInput($_POST['dpt_desempeñar'] ?? '')),
        'sede' => strtoupper(sanitizeInput($_POST['sede'] ?? '')),
        'dependencia' => strtoupper(sanitizeInput($_POST['dependencia'] ?? '')),
        'fecha_ingreso' => $_POST['fecha_ingreso'] ?? date('Y-m-d'), // Usar fecha actual si no se proporciona
    ...
    // ===== 2. ASIGNAR VALORES POR DEFECTO PARA CAMPOS VACÍOS =====
    // Si CI está vacío, generar uno temporal
    if (empty($datos['ci'])) {
        $datos['ci'] = 'TEMP-' . date('YmdHis') . '-' . rand(1000, 9999);
    }
    
    // Si nombre está vacío, usar "NO ESPECIFICADO"
    if (empty($datos['primer_nombre'])) {
        $datos['primer_nombre'] = 'NO ESPECIFICADO';
    }
    
    if (empty($datos['primer_apellido'])) {
        $datos['primer_apellido'] = 'NO ESPECIFICADO';
    }
    
    // Asignar valores por defecto para otros campos obligatorios
    ...
    if (empty($datos['estado_civil'])) {
        $datos['estado_civil'] = 'NO ESPECIFICADO';
    }
    
    if (empty($datos['tipo_trabajador'])) {
        $datos['tipo_trabajador'] = 'NO ESPECIFICADO';
    }
    
    if (empty($datos['grado_instruccion'])) {
        $datos['grado_instruccion'] = 'NO ESPECIFICADO';
    }
    
    if (empty($datos['cargo'])) {
        $datos['cargo'] = 'NO ESPECIFICADO';
    }
    
    if (empty($datos['sede'])) {
        $datos['sede'] = 'NO ESPECIFICADO';
    }
    
    if (empty($datos['dependencia'])) {
        $datos['dependencia'] = 'NO ESPECIFICADO';
    }
    
    if (empty($datos['lateralidad'])) {
        $datos['lateralidad'] = 'NO ESPECIFICADO';
    }
    
    // Si fecha de nacimiento está vacía, usar una fecha por defecto (hace 25 años)
    if (empty($datos['fecha_nacimiento'])) {
        $fecha_nacimiento_default = new DateTime();
        $fecha_nacimiento_default->modify('-25 years');
        $datos['fecha_nacimiento'] = $fecha_nacimiento_default->format('Y-m-d');
    }
    
    // Si fecha de ingreso está vacía, usar fecha actual
    ...
    $camposOpcionales = [
        ...
        'municipio_id', 'parroquia_id', 'dpt_desempeñar'
    ];
    ...
    // ===== 4. DATOS ADICIONALES PARA INACTIVOS =====
    ...
    if (isset($_POST['cedula_familiar']) && is_array($_POST['cedula_familiar'])) {
        foreach ($_POST['cedula_familiar'] as $index => $cedula) {
            // Solo agregar familiar si tiene algún dato
            $nombre = strtoupper(sanitizeInput($_POST['nombre_familiar'][$index] ?? ''));
            $apellido = strtoupper(sanitizeInput($_POST['apellido_familiar'][$index] ?? ''));
            
            if (!empty(trim($cedula)) || !empty($nombre) || !empty($apellido)) {
                $familiares[] = [
                    'cedula' => sanitizeInput($cedula) ?: null,
                    'nombre_familiar' => $nombre ?: null,
                    'apellido_familiar' => $apellido ?: null,
    ...
    // ===== 6. ELIMINAR VALIDACIONES OBLIGATORIAS =====
    $errores = [];
    
    // NOTA: Se han eliminado todas las validaciones obligatorias
    // Todos los campos ahora son opcionales
    
    // Solo validar CI único si se proporcionó un CI (no el temporal)
    if (!empty($_POST['ci']) && $datos['ci'] != 'TEMP-' . date('YmdHis') . '-' . rand(1000, 9999)) {
    ...
    if (empty($errores)) {
        ...
        try {
            ...
            $sql_empleado = "INSERT INTO empleados (
                ...
                cuenta_bancaria, tipo_trabajador, grado_instruccion, cargo, dpt_desempeñar, sede, dependencia,
            ...
            $stmt_empleado->bind_param(
                "sssssssssssssssssssssssssssssssssssiii",
                $datos['nacionalidad'], 
                $datos['ci'], 
                $datos['primer_nombre'], 
            ...
            if (!empty($familiares)) {
                ...
                foreach ($familiares as $familiar) {
                    ...
                    // Si no hay parentesco especificado, usar "OTRO"
                    if (empty($parentesco)) {
                        $parentesco = 'OTRO';
                    }

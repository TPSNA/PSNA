document.addEventListener('DOMContentLoaded', () => {
    // ============================================
    // SCRIPT MODIFICADO: SIN VALIDACIÓN OBLIGATORIA
    // ============================================
    ...
    // ============================================
    // VALIDACIÓN DE PASO - SIN VALIDACIÓN OBLIGATORIA
    // ============================================
    function validateStep(step = 'all') {
        // No hay validación obligatoria, todos los campos son opcionales
        return true; // Siempre retorna true ya que todos son opcionales
    }

    // Navegación entre pasos
    nextBtn.addEventListener('click', () => {
        // Siempre permite avanzar sin validar
        if (currentStep < 5) {
            currentStep++;
            updateSteps();
        }
    ...
    steps.forEach(step => {
        step.addEventListener('click', () => {
            ...
            if (stepNum < currentStep) {
                ...
                // Permitir avanzar sin confirmación
                currentStep = stepNum;
                updateSteps();
            }
        });
        ...
        familiarDiv.innerHTML = `
            ...
            <div class="form-grid">
                <div class="input-group">
                    ...
                    <input type="text" id="cedula_familiar_${familiarCounter}" name="cedula_familiar[]" placeholder="Ej: V-12345678 (Opcional)">
                </div>
                <div class="input-group">
                    <label for="nombre_familiar_${familiarCounter}"><i class="fas fa-user"></i> Nombre</label>
                    <input type="text" id="nombre_familiar_${familiarCounter}" name="nombre_familiar[]" placeholder="Nombre del familiar (Opcional)">
                </div>
                <div class="input-group">
                    <label for="apellido_familiar_${familiarCounter}"><i class="fas fa-user"></i> Apellido</label>
                    <input type="text" id="apellido_familiar_${familiarCounter}" name="apellido_familiar[]" placeholder="Apellido del familiar (Opcional)">
                </div>
                <div class="input-group">
                ...
                <div class="input-group">
                    ...
                    <input type="number" id="edad_${familiarCounter}" name="edad[]" min="0" max="120" placeholder="Edad (Opcional)">
                </div>
                <div class="input-group">
                    <label for="peso_${familiarCounter}"><i class="fas fa-weight"></i> Peso (kg)</label>
                    <input type="number" id="peso_${familiarCounter}" name="peso[]" step="0.1" min="0" max="200" placeholder="Peso en kg (Opcional)">
                </div>
                <div class="input-group">
                    <label for="altura_${familiarCounter}"><i class="fas fa-ruler-vertical"></i> Altura (cm)</label>
                    <input type="number" id="altura_${familiarCounter}" name="altura[]" step="0.1" min="0" max="250" placeholder="Altura en cm (Opcional)">
                </div>
                <div class="input-group">
                    <label for="talla_zapato_${familiarCounter}"><i class="fas fa-shoe-prints"></i> Talla Zapato</label>
                    <input type="number" id="talla_zapato_${familiarCounter}" name="talla_zapato[]" min="20" max="50" placeholder="Talla (Opcional)">
                </div>
                <div class="input-group">
                    <label for="talla_camisa_${familiarCounter}"><i class="fas fa-tshirt"></i> Talla Camisa</label>
                    <input type="text" id="talla_camisa_${familiarCounter}" name="talla_camisa[]" placeholder="Ej: S, M, L, XL (Opcional)">
                </div>
                <div class="input-group">
                    <label for="talla_pantalon_${familiarCounter}"><i class="fas fa-tshirt"></i> Talla Pantalón</label>
                    <input type="text" id="talla_pantalon_${familiarCounter}" name="talla_pantalon[]" placeholder="Ej: 30, 32, 34 (Opcional)">
                </div>
                <div class="input-group">
    ...
    // ============================================
    // VALIDACIÓN DE FECHAS (SOLO SI SE INGRESAN)
    // ============================================
    function validarFechas() {
        ...
        if (fechaNacimiento && fechaIngreso && fechaNacimiento.value && fechaIngreso.value) {
            ...
            // Validar edad mínima (18 años) solo si fecha nacimiento existe
            const hoy = new Date();
    ...
    form.addEventListener('submit', function(e) {
        ...
        const confirmMessage = '¿Está seguro de registrar este trabajador?' + 
            (numFamiliares > 0 ? `\nSe registrarán ${numFamiliares} familiar(es).` : '');
    ...
    // ============================================
    // CARGAR MUNICIPIOS Y PARROQUIAS (AJAX)
    // ============================================
    $(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#municipio').html('<option value="">Selecciona un municipio (Opcional)</option>' + data);
            },
            ...
            if (municipioId) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#parroquia').html('<option value="">Selecciona una parroquia (Opcional)</option>' + data);
                    },
                    error: function(xhr, status, error) {
                ...
                $('#parroquia').html('<option value="">Selecciona una parroquia (Opcional)</option>');
            }
        });
        ...
        function cargarMunicipiosEstaticos() {
            ...
            let options = '<option value="">Selecciona un municipio (Opcional)</option>';
        ...
        function cargarParroquiasEstaticas(municipioId) {
            ...
            let options = '<option value="">Selecciona una parroquia (Opcional)</option>';
    ...
    if (successModal) {
        ...
        // Iniciar cuenta regresiva y confetti
        startCountdown();
        createConfetti();
    }
    ...
    console.log('Sistema de formulario inicializado correctamente - TODOS LOS CAMPOS SON OPCIONALES');
});
